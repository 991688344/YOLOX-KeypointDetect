...
class ONNXPredictor:
    ...
    def inference_batch(self, img_list):
        """
        批量推理多张图像。
        Args:
            img_list: list of numpy array (H,W,3) BGR 图像
        Returns:
            outputs_list: list of postprocess results (每个元素与单张 inference 返回的 outputs[0] 结构相同)
            img_list: 原图列表（未修改）
        """
        if not img_list:
            return [], []
        batch_size = len(img_list)
        # 预处理每帧，收集 letterbox_info
        preprocessed_list = []
        letterbox_infos = []
        for img in img_list:
            h, w = img.shape[:2]
            img_processed, ratio, dx, dy = preprocess(img, self.input_size)
            preprocessed_list.append(img_processed)
            letterbox_infos.append({
                "img_h": h, "img_w": w, "r": ratio, "dx": dx, "dy": dy,
                "input_h": self.input_size[0], "input_w": self.input_size[1]
            })
        # 堆叠成 batch
        batch_input = np.stack(preprocessed_list, axis=0)  # (batch, 3, H, W)
        # 执行推理
        batch_outputs = self.session.run(None, {self.input_name: batch_input})
        # 将 batch 输出拆解为每个样本的原始预测
        # batch_outputs 每个元素 shape: (batch, ...)
        outputs_list = []
        for i in range(batch_size):
            single_outputs = [out[i:i+1] for out in batch_outputs]  # 每个 shape (1, ...)
            prediction = decode_outputs(single_outputs, self.num_classes)  # (1, num_boxes, ...)
            # 后处理
            post_out = postprocess(prediction, letterbox_infos[i], self.num_classes,
                                   self.conf_thre, self.nms_thre, False, self.keypoints, False)
            outputs_list.append(post_out[0])  # post_out 是 list，取第一个元素
        return outputs_list, img_list
...
def process_one_video(video_path, predictor, args):
    ...
    while True:
        ...
        if not ret:
            # 处理剩余帧
            if frames_batch:
                outputs_list, _ = predictor.inference_batch(frames_batch)
                for out, idx, frm in zip(outputs_list, indices_batch, frames_batch):
                    if args.error_analysis and out is not None:
                        dets_np = out.cpu().numpy()
                        cls_ids = dets_np[:, 6].astype(int)
                        if 4 in cls_ids:
...
def make_parser():
    ...
    # 批处理参数
    parser.add_argument("--batch_size", default=4, type=int, help="批处理大小，增大可加速 GPU 推理（默认4）")
    return parser
