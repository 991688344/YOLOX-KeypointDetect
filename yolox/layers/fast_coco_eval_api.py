...
class COCOeval_opt(COCOeval):
    ...
    def evaluate(self):
        ...
        if p.iouType == "segm" or p.iouType == "bbox":
            ...
            gt = self._gts[p.imgIds[0], catIds[0]][0]
            assert len(gt['keypoints']) == k * 3, \
                f"GT keypoints {len(gt['keypoints'])//3} != sigmas {k}"
    ...
    def setKpParams(self, num_keypoints=None, kpt_oks_sigmas=None):
        self.imgIds = []
        self.catIds = []
        ...
        if kpt_oks_sigmas is not None:
            # 显式传入 sigmas
            sigmas = np.asarray(kpt_oks_sigmas, dtype=np.float32)
        elif num_keypoints is not None:
            # 仅给定关键点数量：使用统一默认 sigma
            # 0.5 / 10
            sigmas = np.full(
                shape=(num_keypoints,),
                fill_value=0.5,
        ...
        # COCO 规范：sigma 要除以 10
        self.params.kpt_oks_sigmas = sigmas / 10.0
