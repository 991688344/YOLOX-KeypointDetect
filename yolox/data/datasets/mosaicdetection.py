...
from yolox.utils import adjust_box_anns, get_local_rank, adjust_lmks_anns
...
class MosaicDetection(Dataset):
    ...
    def __init__(
        ...
        degrees=10.0, translate=0.1, mosaic_scale=(0.5, 1.5), keypoints=0, segcls=0,
    ...
    def __getitem__(self, idx):
        ...
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            for i_mosaic, index in enumerate(indices):
                img, _labels, _, img_id, seg = self._dataset.pull_item(index)
            ...
            mosaic_img, mosaic_labels, mosaic_seg = random_perspective(
                mosaic_img,
                mosaic_labels,
            ...
            img, label, img_info, img_id, seg = self._dataset.pull_item(idx)
            img, label, seg = self.preproc(img, label, self.input_dim, seg)
            return img, label, img_info, img_id, seg
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        img, cp_labels, _, _, seg_target = self._dataset.pull_item(cp_index)  # !!!!
        ...
        if self.keypoints > 0:
            cp_landmarks_origin_np = adjust_lmks_anns(
                cp_labels[:, -2*self.keypoints:], cp_scale_ratio, 0, 0, origin_w, origin_h
            )
        if FLIP:
            ...
            if self.keypoints > 0:
                ...
                cp_landmarks_origin_np *= (cp_landmarks_origin_np > 0)

        cp_bboxes_transformed_np = cp_bboxes_origin_np.copy()
        cp_landmarks_transformed_np = cp_landmarks_origin_np.copy() if self.keypoints > 0 else np.array([])
